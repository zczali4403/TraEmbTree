#!/usr/bin/env python3
# Embedding-only MC2-inspired selection with hard lineage x exact-stage strata.
#
...
#     /mnt/input/sc_cz/Concord/eval/2026_08_19/select_landmark_nodes_mc2_lineage_stage.py \
#     --target-nodes 2000 \
#     --output-dir /mnt/input/sc_cz/Concord/eval/2026_08_19/landmark_nodes_mc2_lineage_stage \
#     --overwrite
#
# Quick small test:
#   .../python select_landmark_nodes_mc2_lineage_stage.py --max-cells 100000 --target-nodes 100 \
...
def args_parser():
    ...
    p.add_argument('--output-dir',type=Path,default=HERE/'landmark_nodes_mc2_lineage_stage')
...
def main():
    ...
    try:
        ...
        quota=allocate(stratum_counts,a.target_nodes,a.min_nodes_per_stratum,a.quota_power)
        ...
        all_micro_groups=[]; micro_lineages=[]; micro_constraint_stages=[]; micro_to_node=[]
        rng=np.random.default_rng(a.seed)
        offset=0; log(f'input={n:,} x {dim}; nonempty lineage-stage strata={len(unique_keys):,}; final nodes={a.target_nodes}')
        for stratum_index,(key,start,count) in enumerate(zip(unique_keys,starts,stratum_counts)):
            ...
            ids=stratum_order[int(start):int(start+count)]; q=int(quota[stratum_index])
            log(f'stratum {stratum_index+1}/{len(unique_keys)}: lineage={name}; stage={stage_value:g}; cells={len(ids):,}; target_nodes={q}')
            ...
            for i,members in enumerate(phase2):
                ...
                micro_lineages.append((lid,name)); micro_constraint_stages.append((sid,stage_value)); micro_to_node.append(node)
            for g in range(q):
                members=np.concatenate([phase2[i] for i in np.flatnonzero(final_lab==g)]); node=offset+g; assignment[members]=node; all_node_groups.append(members); node_lineages.append((lid,name)); node_constraint_stages.append((sid,stage_value))
