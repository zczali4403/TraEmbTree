#!/usr/bin/env python3
# Embedding-only MC2-inspired landmark selection.
#
# Example (1000 final nodes, all 15.2M cells):
#   OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 \
#   /home/aiscuser/.conda/envs/train/bin/python \
#     /mnt/input/sc_cz/Concord/eval/2026_08_19/select_landmark_nodes_mc2.py \
#     --target-nodes 1000 \
#     --output-dir /mnt/input/sc_cz/Concord/eval/2026_08_19/landmark_nodes_mc2 \
#     --overwrite
#
# Quick small test:
#   .../python select_landmark_nodes_mc2.py --max-cells 100000 --target-nodes 100 \
#     --output-dir landmark_nodes_mc2_test --overwrite
"""MC2-inspired divide-and-conquer landmark selection using only TraEmb.
...
def args_parser():
    ...
    p.add_argument('--output-dir',type=Path,default=HERE/'landmark_nodes_mc2')
    p.add_argument('--target-nodes',type=int,default=1000)
...
def allocate(counts,total,minimum,power):
    ...
    if total<n: raise ValueError(f'target-nodes must be >= nonempty lineages ({n})')
...
def main():
    ...
    try:
        ...
        stages=np.asarray(md['stages'],dtype=np.float32); counts=np.bincount(lineage,minlength=len(names)); quota=allocate(counts,a.target_nodes,a.min_nodes_per_lineage,a.quota_power)
        ...
        all_node_groups=[]; node_lineages=[]
        all_micro_groups=[]; micro_lineages=[]; micro_to_node=[]
        rng=np.random.default_rng(a.seed)
        offset=0; log(f'input={n:,} x {dim}; final nodes={a.target_nodes}')
        for lid,name in enumerate(names):
            ids=np.flatnonzero(lineage==lid); q=int(quota[lid])
            if not len(ids):continue
            log(f'lineage {name}: cells={len(ids):,}, target_nodes={q}')
            ...
            if len(phase2)<q:
                raise RuntimeError(f'{name}: phase2 produced {len(phase2)} microcells for {q} requested nodes; reduce --microcell-size')
            micro_centers=raw_means(emb,phase2); weights=np.asarray([len(g) for g in phase2]); ideal=max(1,int(math.ceil(len(ids)/q)))
            ...
            for i,members in enumerate(phase2):
                micro_id=len(all_micro_groups); node=offset+int(final_lab[i])
                micro_assignment[members]=micro_id; all_micro_groups.append(members)
                micro_lineages.append((lid,name)); micro_to_node.append(node)
            for g in range(q):
                members=np.concatenate([phase2[i] for i in np.flatnonzero(final_lab==g)]); node=offset+g; assignment[members]=node; all_node_groups.append(members); node_lineages.append((lid,name))
            offset+=q; log(f'  finalized {len(phase2):,} microcells -> {q} nodes')
        ...
        for node,g in enumerate(all_node_groups):
            ...
            lid,name=node_lineages[node]; row=dict(node_id=node,lineage=name,lineage_id=lid,n_cells=len(g),mean_stage=float(st.mean()),stage_std=float(st.std()),min_stage=float(st.min()),max_stage=float(st.max()),mean_cosine_distance=float((1-sim).mean()),p95_cosine_distance=float(np.quantile(1-sim,.95)))
        ...
        for micro_id,g in enumerate(all_micro_groups):
            ...
            lid,name=micro_lineages[micro_id]; ct=dict(mctmap[micro_id]); ct.pop('node_id',None)
            row=dict(microcell_id=micro_id,landmark_node_id=int(micro_to_node[micro_id]),lineage=name,lineage_id=lid,n_cells=len(g),mean_stage=float(st.mean()),stage_std=float(st.std()),min_stage=float(st.min()),max_stage=float(st.max()),mean_cosine_distance=float((1-sim).mean()),p95_cosine_distance=float(np.quantile(1-sim,.95)))
        ...
        cfg={k:str(v) if isinstance(v,Path) else v for k,v in vars(a).items()};cfg.update(n_cells=n,n_nodes=len(rows),n_microcells=len(micro_rows),celltype_matched=matched,microcell_celltype_matched=mmatched,method='embedding_only_mc2_inspired_cosine_knn_two_pass')
